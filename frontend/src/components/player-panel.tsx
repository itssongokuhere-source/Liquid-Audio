...
export type PanelTab = "upnext" | "lyrics" | "related";
  ...
  return (
    <View style={styles.container} testID="queue-screen">
      <View style={styles.top}>
        <AnimatedPressable testID="queue-close" onPress={onClose} style={styles.iconBtn}>
          <Icon name="chevron-down" size={28} color={WHITE} />
        </AnimatedPressable>
        <Text style={styles.title}>{tab === "upnext" ? "Up Next" : tab === "lyrics" ? "Lyrics" : "Related"}</Text>
        <AnimatedPressable
          testID="queue-clear"
          onPress={() => {
            clearUpcoming();
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "rgba(12,12,14,0.96)", borderTopLeftRadius: 28, borderTopRightRadius: 28, overflow: "hidden" },
  top: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingTop: 6, paddingBottom: 8 },
